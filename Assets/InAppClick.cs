...
public class InAppClick : MonoBehaviour, IStoreListener
{
    ...
    public static string[] Products = new string[] { "beginner_4","medium_8","pro_15","bizz_25", "starter_kit","starter_kit_4", "starter_kit_8" };
    ...
    public void BuyConsumable_subs_8()
    {
        // Buy the consumable product using its general identifier. Expect a response either 
        // through ProcessPurchase or OnPurchaseFailed asynchronously.
        BuyProductID(Products[5]);
    }
    public void BuyConsumable_subs_15()
    {
        // Buy the consumable product using its general identifier. Expect a response either 
        // through ProcessPurchase or OnPurchaseFailed asynchronously.
        BuyProductID(Products[6]);
    }
    public void BuyConsumable_subs_25()
    {
        // Buy the consumable product using its general identifier. Expect a response either 
        // through ProcessPurchase or OnPurchaseFailed asynchronously.
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (String.Equals(args.purchasedProduct.definition.id, Products[0], StringComparison.Ordinal))
        {
            ...
            var url = "https://gamelovin.com/tradeoption/Trading/Userprocess/UpdateInApp";
        ...
        else if (String.Equals(args.purchasedProduct.definition.id, Productssubscribe[0], StringComparison.Ordinal))
        {
            Debug.Log(string.Format("ProcessPurchase: PASS. Product: '{0}'", args.purchasedProduct.definition.id));
            // Debug.Log(string.Format("ProcessPurchase: PASS. Product: '{0}'", args.purchasedProduct.definition.id));
            coins = 2500 * mainui.sale;
            var url = "https://gamelovin.com/tradeoption/Trading/Userprocess/UpdateInApp";
            string json = "{\"unique_id\":\"" + PlayerPrefs.GetString("uniqueid") + "\",\"pack\":\"beginner_subs_4\",\"coins\":\""+coins+"\",\"usd\":\"4\"}";
            PlayerPrefs.SetInt("subscriber", 1);
            StartCoroutine(PostRequestrewardsCoroutine(url, json));
        }
